...
Inference:
    $ python MainYoloDetect.py --weights yolov5s.pt                 # PyTorch
...
from YoloImpl.models.experimental import attempt_load
from YoloImpl.models.yolo import ClassificationModel, Detect, DetectionModel, SegmentationModel
from YoloImpl.utils.dataloaders import LoadImages
from YoloImpl.utils.general import (LOGGER, Profile, check_dataset, check_img_size, check_requirements, check_version,
...
def run(
    ...
    if any(f):
        ...
        LOGGER.info(f'\nExport complete ({time.time() - t:.1f}s)'
                    ...
                    f"\nDetect:          python {dir / ('MainYoloDetect.py' if det else 'predict.py')} --weights {f[-1]} {h}"
